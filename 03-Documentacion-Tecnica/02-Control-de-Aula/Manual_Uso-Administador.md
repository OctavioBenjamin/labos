...
## Acceso al Menú
Para acceder al menú, primero debes conectarte al servidor mediante SSH. Si aún no tienes configurada tu conexión, sigue este tutorial: [Configurar Sesión SSH](./Manual_Configurar_Sesion-ssh.md)

Una vez conectado al servidor, el menú se puede iniciar con:
```bash
bash 02-Software-Admin/menu_admin-labos.sh
```
*(Nota: El usuario `controlador` debe tener este menú configurado para iniciar automáticamente al entrar).*
---
...
- **Ver estado de equipos (Ping):** Identifica cuales son los equipos que están encendidos. 
